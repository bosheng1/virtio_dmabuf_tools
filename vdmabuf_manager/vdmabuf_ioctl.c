...
int ioctl_alloc(int fd, uint32_t size)
{
	...
	if (ret < 0) {
		ERROR("failed to alloc, ret:%d", ret);
		return ret;
	}
	return attr.fd;
}
...
int ioctl_attach(int fd, char *name)
{
	...
	if (ret < 0) {
		ERROR("failed to attach VM:%s, ret:%d", name, ret);
		return ret;
	}
	return ret;
}
...
int ioctl_export(int fd, int bo_fd, int priv_size, char *priv, virtio_vdmabuf_buf_id_t *buf_id)
{
	...
	if (ret < 0) {
		ERROR("failed to export, ret:%d", ret);
		return ret;
	}
...
int ioctl_unexport(int fd, virtio_vdmabuf_buf_id_t *buf_id)
{
	...
	if (ret < 0) {
		ERROR("failed to unexport, ret:%d", ret);
		return ret;
	}
	return ret;
}
...
int ioctl_import(int fd, virtio_vdmabuf_buf_id_t *buf_id)
{
	...
	if (ret < 0) {
		ERROR("failed to import, ret:%d", ret);
		return ret;
	}
	return attr.fd;
}
...
int ioctl_query_bufinfo(int fd, virtio_vdmabuf_buf_id_t *buf_id, int * size, int *priv_size, char *data)
{
	...
	if (ret < 0) {
		ERROR("failed to query bufinfo, ret:%d", ret);
		return ret;
	}
	*size = attr.info;
